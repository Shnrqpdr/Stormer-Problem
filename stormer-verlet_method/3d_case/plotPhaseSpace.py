...
fileProton1=pd.read_csv("phaseSpaceP1.dat", header = 0, sep='\s+')
#fileProton2=pd.read_csv("phaseSpace2.dat", header = 0, sep='\s+')
#fileProton3=pd.read_csv("phaseSpace3.dat", header = 0, sep='\s+')
...
'''Proton 1'''
ax1.plot(fileProton1['rho'],fileProton1['drho'], '-', label=label_1)
...
'''Próton 2'''
#ax1.plot(fileProton2['rho'],fileProton2['drho'], '-', label=label_2, color='mediumseagreen')
...
'''Proton 3'''
#ax1.plot(fileProton3['rho'],fileProton3['drho'], '-', label=label_3)
...
#plt.text(-0.55, -0.15, 'Terra', fontsize = 8) #-0.45 e -0.15 fontsize = 15 é o padrão para plotar com 2 protons
# plt.yscale("log")
# plt.xscale('log')
